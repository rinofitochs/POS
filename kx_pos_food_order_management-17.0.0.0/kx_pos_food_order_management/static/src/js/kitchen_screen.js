...
class kitchen_display_order_dashboard extends Component {
    ...
    done_order(e) {
        var self = this;
        var input_id = $("#" + e.target.id).val();
        this.orm.call("pos.order", "order_status_update", [Number(input_id)])
        var current_order = this.state.order_details.filter((order) => order.id==input_id)
         if(current_order){
    ...
    accept_order_line(e) {
        var input_id = $("#" + e.target.id).val();
        this.orm.call("pos.order.line", "order_status_update", [Number(input_id)])
...
kitchen_display_order_dashboard.template = 'KitchenDisplayCustom';
registry.category("actions").add("kitchen_custom_dashboard_tags", kitchen_display_order_dashboard);